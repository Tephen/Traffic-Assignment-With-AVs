...
def computeIttc(network: FlowTransportNetwork, link):
    ...
    计算瞬时总行程时间差
    """
    original_total_time = network.mixLinkSet[link].cost * (
                network.mixLinkSet[link].av_flow + network.mixLinkSet[link].cv_flow) + \
    ...
    # 部分av车辆移动到avlink
        ...
        b = (((network.mixLinkSet[link].lane_num - 1) * network.hav * (
                    network.mixLinkSet[link].av_flow + network.avLinkSet[link].flow)) \
...
def updateTravelTime(network: FlowTransportNetwork, optimal: bool = False, costFunction=BPRcostFunction):
    ...
    for l in network.mixLinkSet:
        ...
        network.mixLinkSet[l].capacity = (3600.0 / ((network.hav * ra) + (network.hcv * (1 - ra)))) * \
                                         network.mixLinkSet[l].lane_num
...
def loadAON(network: FlowTransportNetwork, computeXbar: bool = True):
    ...
    for r in network.originZones:
        ...
        for s in network.zoneSet[r].destList:
            ...
            av_rate = network.tripSet[r, s].av_rate
            av_dem = dem * av_rate
            cv_dem = dem * (1 - av_rate)
            if dem <= 0:
                continue
...
def readDemand(demand_df: pd.DataFrame, network: FlowTransportNetwork):
    # 计算平均avRate
    network.avgAvRate = demand_df['av_rate'].mean()
    for index, row in demand_df.iterrows():
        ...
        av_rate = row["av_rate"]
        # print(row)
        network.tripSet[init_node, term_node] = Demand(init_node, term_node, demand, av_rate)
...
def assignment_loop(network: FlowTransportNetwork,
    ...
    while gap > accuracy:
        ...
        AV_TSTT = round((sum([network.avLinkSet[a].flow * network.avLinkSet[a].cost for a in network.avLinkSet])
                         + sum(
                    [network.mixLinkSet[b].av_flow * network.mixLinkSet[b].cost for b in network.mixLinkSet])), 3)
...
def writeResults(deployIter, network: FlowTransportNetwork, output_file: str, TSTT: float, costFunction=BPRcostFunction,
    ...
    with open(output_file, "a", encoding="utf-8-sig") as csvFile:
        ...
        tmpOut = ["当前阶段AV平均市场率: " + str(network.avgAvRate) + "\t当前阶段AV车道长度百分比限制: " + str(
            network.avgAvRate)]
        outFile.writerow(tmpOut)
...
def deploy_loop(network: FlowTransportNetwork,
    ...
    network.avLengthLimit = network.avgAvRate
    timeToUE = 0
    deployNum = 0
    while True:
        # 1. 求解UE问题
        ...
        for mix_l in network.mixLinkSet:
            ...
            if (network.mixLinkSet[mix_l].lane_num > 1) and \
                    ((network.mixLinkSet[
                          mix_l].length + network.avLinkTotalLength) / network.linkTotalLength <= network.avLengthLimit):
                # 只保留瞬时tt变化<0的备选车道位置
        ...
        if len(candidateDict) == 0:
            if verbose:
                ...
                print(
                    f"AV车道限长为{network.linkTotalLength * network.avLengthLimit},当前AV车道总长已达{network.avLinkTotalLength}")
                print("没有可部署车道")
                break
...
if __name__ == '__main__':
    ...
    results_file = '_'.join(net_file.split("_")[:-1] + ["heterogeneous_flow.csv"])
